...
defmodule AutoApi.CapabilityParser do
  ...
  defp bin_to_map_5(<<0x01, size::integer-16, capability::binary-size(size), rest_data::binary>>) do
    ...
    with {_id, cap_module} <- find_cap_module_by_iden(iden),
         ...
         actions_in_atom when is_list(actions_in_atom) <-
           supported_actions_to_atoms(cap_module, :binary.bin_to_list(supported_actions)),
  ...
  defp bin_to_map(<<iden::binary-size(2), size, cap_data::binary-size(size), rest_data::binary>>) do
    with {_id, cap_module} <- find_cap_module_by_iden(iden),
         ...
         map_cap when is_map(map_cap) <- bin_to_map(rest_data) do
  ...
  def to_bin(map, :v4) do
    ...
    caps_list =
      Enum.map(map, fn {cap_group, caps} ->
  ...
  def to_bin(map, :v5) do
    ...
    caps_list =
      map
      |> Enum.map(fn {cap_group, caps} ->
        padd_capabilities_bin(name_to_module[cap_group], caps)
      end)
      |> :binary.list_to_bin()
