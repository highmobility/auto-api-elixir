...
defmodule AutoApiL11.CommandTest do
  use ExUnit.Case
  doctest AutoApiL11.Command

  alias AutoApiL11.{Capability, PropertyComponent}
  ...
  describe "to_bin/2" do
    test "get works with all properties and all capabilities" do
      results =
        ...
        |> Enum.reject(fn {_cap, command} -> command == AutoApiL11.NotImplemented end)
    ...
    test "set works" do
      ...
      bin_command = AutoApiL11.DoorsCommand.to_bin(:set, properties)
    ...
    test "setter supports constants" do
      assert <<0x0B, 0, 34, 1, 1, 0, 4, 1, 0, 1, 0>> == AutoApiL11.WakeUpCommand.to_bin(:wake_up)
    end
  end
  ...
  describe "to_bin/2 and from_bin/2 are inverse functions" do
    test ":set" do
      ...
      assert cmd_bin = AutoApiL11.DiagnosticsCommand.to_bin(:set, properties)
      assert {:set, properties} == AutoApiL11.DiagnosticsCommand.from_bin(cmd_bin)
    end

    test ":get" do
      properties = [:speed, :tire_pressures, :engine_rpm]

      assert cmd_bin = AutoApiL11.DiagnosticsCommand.to_bin(:get, properties)
      assert {:get, properties} == AutoApiL11.DiagnosticsCommand.from_bin(cmd_bin)
    end

    test ":get with no properties" do
      assert cmd_bin = AutoApiL11.DiagnosticsCommand.to_bin(:get)
      assert {:get, []} == AutoApiL11.DiagnosticsCommand.from_bin(cmd_bin)
    end
  end

  describe "execute/2" do
    test "set overwrites multiple properties" do
      state = %AutoApiL11.DoorsState{
        locks: [
      ...
      command_bin = AutoApiL11.DoorsCommand.to_bin(:set, command_props)

      assert AutoApiL11.DoorsCommand.execute(state, command_bin) ==
               %AutoApiL11.DoorsState{
                 locks: [
